...
const ScaleComponent: React.FC<ScaleComponentProps> = (props) => {
    ...
    const limitThreshole = (value: number) =>
        range(value, defaultSetting.start, defaultSetting.end)
    ...
    const scrollAction = (distance: number, _duration: number) => {
        ...
        const step = () => {
            let value = easeOut(
                currentTime,
                originVal,
                targetDistance - originVal,
                duration,
            )
            if (currentTime < duration) {
                startVal.current =
                    Math.round(
                        limitThreshole(value) / defaultSetting.precision,
    ...
    const drawMiddleLine = (context: CanvasRenderingContext2D) => {
        /** ————绘制中间线———— */
        const midLineXVal = Math.floor(
            originCanvasInfo.current.originCanvasWidth / 2,
        )
        context.beginPath()
        context.lineWidth = 4
    ...
    const drawScale = useCallback(() => {
        ...
        let beginNum =
            startVal.current -
            (originCanvasWidth / 2 / defaultSetting.lineMargin) *
                defaultSetting.precision
        /** 当前能绘制刻度尺的总数 */
        let scaleTotal = (originCanvasWidth / defaultSetting.lineMargin) | 0
        /** 当前刻度值与向上取整的刻度值之间的差值 */
        let beginNumDiffVal =
            Math.ceil(beginNum / defaultSetting.precision) *
                defaultSetting.precision -
            beginNum
        /** 计算出间距与精度之间的比例值 */
        const marginPrecisionRatio =
            defaultSetting.lineMargin / defaultSetting.precision
        /** 需要空出来的位移值 */
        const blankMoveVal = beginNumDiffVal * marginPrecisionRatio
        for (let i = 0; i < scaleTotal; i++) {
            let currentNum =
                Math.ceil(beginNum / defaultSetting.precision + i) *
                defaultSetting.precision
    ...
    useEffect(() => {
        ...
        const drawScaleInit = () => {
            ...
            const { width: originCanvasWidth, height: originCanvasHeight } =
    ...
    const onTouchMove = (event: TouchEvent | React.TouchEvent) => {
        ...
        startVal.current = range(
            startVal.current -
                (move_x / defaultSetting.lineMargin) * defaultSetting.precision,
    ...
    const onTouchEnd = (event: TouchEvent | React.TouchEvent) => {
        ...
        const distance =
            (event.changedTouches[0].pageX - touchInfo.current.startX) *
            defaultSetting.precision
        const allowEaseAction =
            duration < MOMENTUM_LIMIT_TIME &&
            Math.abs(distance) >
                MOMENTUM_LIMIT_DISTANCE * defaultSetting.precision
        if (allowEaseAction) {
            scrollAction(distance, duration)
        } else {
            startVal.current =
                Math.round(
                    limitThreshole(startVal.current) / defaultSetting.precision,
                ) * defaultSetting.precision
            window.requestAnimationFrame(() => drawScale())
        }
    }
